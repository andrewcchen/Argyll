...
void usage(char *diag) {
	...
	fprintf(stderr," -g          output fake RGB values instead of CMY\n");
	fprintf(stderr,"profile%s  CMYK  ICC input profile to use\n",ICC_FILE_EXT);
	fprintf(stderr,"outfile.ti3  Name for output .ti3 file\n");
	exit(1);
}
...
int main(int argc, char *argv[]) {
	...
	int verb = 0;
	int dorgb = 0;			/* Create fake RGB values */
		...
		for (i = 0; i < fxno; i++) {
			if (dorgb) {
				printf("Point %d = %f %f %f -> %f %f %f\n",i,
				        1.0 - fxlist[i].p[0], 1.0 - fxlist[i].p[1], 1.0 - fxlist[i].p[2],
				        fxlist[i].v[0], fxlist[i].v[1], fxlist[i].v[2]);
			} else {
				printf("Point %d = %f %f %f -> %f %f %f\n",i,
